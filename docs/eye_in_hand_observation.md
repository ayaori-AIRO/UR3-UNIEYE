...
HOME 이동량 제한은 `--home-max-joint-travel` (기본 2.15 rad)로 분리했습니다.
home/sequence/full의 HOME 단계에만 적용하며 성공/실패/예외 후 원래 제한을 복원합니다.
관찰/접근은 `--max-joint-travel` (기본 2.10 rad)을 유지합니다.
이는 예제의 이동량 검사 한도이며 UR3 물리적 관절 한계를 변경하지 않습니다.
제한 변경이 경로의 안전을 보장하지 않으므로 전체 경로와 카메라/케이블 여유를 확인하세요.

## 전체 시나리오 한 번 실행

full은 시작 시 `D405 continuous live` 창을 열어 HOME/관찰/접근 중에도
원본 컬러 영상을 표시합니다. 이 창은 추론이나 이동 명령을 수행하지 않습니다.
검출 프로세스는 `--no-window`로 실행하고 `/scissors/preview_image`를 발행합니다.
상시 창 하나에서 검출 박스와 선택점을 표시합니다. 검출 영상은 촬영 당시의
스냅샷이며 수신 후 1초 동안만 표시하고 원본 라이브로 복귀합니다.
오래된 박스를 새 영상에 겹치지 않습니다. 목표 확정 후에도 라이브 창은 유지됩니다.
정상 완료 후 라이브 창의 q/ESC 또는 터미널 Ctrl+C로 종료합니다.
라이브 창 닫기는 로봇 정지 명령이 아닙니다. 실패/중단 시 소유한 창은 정리됩니다.
`--no-live-view`로 기존처럼 시나리오 종료 시 바로 끝나게 할 수 있습니다.
